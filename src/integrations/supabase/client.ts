...
export async function deleteOfferCompletely(offerId: string) {
  ...
  try {
    // 1. First, find all tracking links associated with this offer
    ...
    if (trackingLinksError) {
      ...
      // 2. Delete tracking links associated with this offer
    ...
    // 3. Find clicks associated with this offer
    const { data: clicks, error: clicksError } = await supabase
      .from('clicks')
      .select('id, click_id')
      .eq('offer_id', offerId);
    ...
    if (clicksError) {
      ...
      // 4. Delete conversions associated with clicks from this offer
      ...
      // 5. Now delete the clicks themselves
    ...
    // 6. Get all affiliate offers for this offer
    ...
    if (affiliateOffersError) {
      ...
      // 7. Direct delete of affiliate offers
      ...
      if (deleteOffersError) {
        console.error("Error deleting affiliate offers:", deleteOffersError);
        
        // Try deleting them one by one if bulk delete fails
        let hasErrors = false;
        for (const ao of affiliateOffers) {
          ...
          if (singleDeleteError) {
            console.error(`Error deleting affiliate offer ${ao.id}:`, singleDeleteError);
            hasErrors = true;
          } else {
    ...
    // 8. Double-check no more affiliate_offers exist for this offer
    ...
    // 9. Finally delete the offer itself
